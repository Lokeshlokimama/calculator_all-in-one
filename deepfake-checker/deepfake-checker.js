(() => {
    ...
    function apiUnavailableError(status) {
        ...
        return {
            ...
            message: 'Deepfake detection provider is not configured on this deployment. Set DEEPFAKE_API_KEY and DEEPFAKE_PROVIDER on hosting that supports the /api/deepfake-checker serverless endpoint.'
        };
    }
    ...
    async function analyzeSelectedFile() {
        ...
        try {
            ...
            if (!response.ok) {
                throw payload?.error || apiUnavailableError(response.status) || { message: `The server returned status ${response.status}. Try again later.` };
            }
